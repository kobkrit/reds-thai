...
exports.createClient = function(fn){
  return fn();
}
 ...
 *
 * @param {String} key, {RedisClient} client
 * @return {Search}
 * @api public
 */

exports.createSearch = function (key, client) {
    if (!key) throw new Error('createSearch() requires a redis key for namespacing');
    if (!client) client = exports.createClient();
    return new Search(key, client);
};
...
exports.words = function (str) {
    var clean = String(str).replace(/[^ก-๛\w]+/g, " ");
    return wordcut.cut(clean).replace(/ /g,"").split(/\|+/g).filter(function(v){return v.length>0});
};
...
Search.prototype.index = function (str, id, fn) {
    ...
    keys.forEach(function (word, i) {
        for (var l = 1; l < map[word].length+1; l++) {
            var pre_word = map[word].substring(0,l);
            var pre_score = counts[word] * l;
            cmds.push(['zincrby', key + ':word:' + pre_word, pre_score, id]);
            cmds.push(['zincrby', key + ':object:' + id, pre_score, pre_word]);
        }
    });
